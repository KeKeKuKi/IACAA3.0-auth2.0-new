<template>
  <div class="app-container">
    <div class="filter-container">
      <el-select v-model="currentCompany" placeholder="请选择公司" @change="loadTree">
        <el-option
...
export default {
  ...
  mounted() {
    requestByClient(Auth, 'get', 'api/company/findAll', null, resp => {
  ...
  methods: {
    loadTree() {
      requestByClient(Auth, 'get', '/api/position/findTree/' + this.currentCompany, null, resp => {
    ...
    handleDragEnd(draggingNode, dropNode, dropType) {
      ...
      requestByClient(Auth, 'post', '/api/position/updateAll', this.data[0].children, resp => {
        if (resp.data.code === 0) {
          this.showSuccess()
        }
      })
    ...
    confirm() {
      this.form.companyId = this.currentCompany
      if (this.isEdit) {
        requestByClient(Auth, 'put', '/api/position', this.form, resp => {
          if (resp.data.code === 0) {
        ...
        requestByClient(Auth, 'post', '/api/position', this.form, resp => {
          const { code, data } = resp.data
          if (code === 0 && data) {
            this.showSuccess(this.form)
    ...
    remove(node, data) {
      requestByClient(Auth, 'delete', '/api/position/' + data.id, null, resp => {
